...
export const UserForm = () => {
  ...
  if (isSubmitted) {
    return (
      <EmailVerification 
        onConfirm={() => {
          localStorage.setItem('userRole', 'user'); // Зберігаємо роль звичайного користувача
          navigate('/home');
        }} 
    ...
    <div className="min-h-screen bg-zinc-900 flex items-center justify-center p-6">
      <div className="w-full max-w-xl bg-white rounded-[40px] p-10 relative shadow-2xl">
        ...
        <div className="flex flex-col items-center mb-8 mt-4">
          ...
          <h2 className="text-3xl font-bold text-black">Користувач</h2>
          <p className="text-gray-400 text-sm font-medium">Приєднуйтесь, щоб підтримувати проєкти</p>
        </div>

        <form className="space-y-5 text-left">
          <div>
            <label className="block text-xs font-bold text-gray-800 mb-1.5 ml-1 uppercase tracking-wider">Ім'я</label>
            <input type="text" placeholder="Ваше ім'я" className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-1 focus:ring-black" />
          </div>
          ...
          <button 
            ...
            className="w-full py-5 bg-black text-white rounded-[20px] font-bold text-base mt-4 shadow-xl hover:bg-zinc-800 transition-all flex items-center justify-center gap-2"
          >
            <span className="text-lg">✓</span> Зареєструватися
          </button>
        </form>
      </div>
    </div>
